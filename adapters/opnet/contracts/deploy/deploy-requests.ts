...
async function main(): Promise<void> {
    ...
    console.log('Operator address:', wallet.p2tr);
    ...
    if (balance === 0n) {
        console.error('No funds at operator address');
        await provider.close();
        process.exit(1);
    }

    const bytecode = readFileSync(
        new URL('../broker-requests/build/BrokerRequests.wasm', import.meta.url),
    );
    ...
    console.log('\nBrokerRequests deployment complete!');
